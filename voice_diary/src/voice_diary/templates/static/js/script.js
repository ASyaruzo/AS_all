document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        alert("音声認識の開始時にエラーが発生しました: " + error.message);
    }
...
async function fetchDiaries(date) {
    try {
        ...
        if (response.ok) {
            ...
            console.warn("日記が見つかりませんでした:", data.message);
            document.getElementById('diaryList').innerHTML = '<div class="no-diary">この日の日記はありません</div>';
        }
    } catch (error) {
...
function displayDiaries(diaries) {
    ...
    diaries.forEach(diary => {
        ...
        diaryElement.innerHTML = `
            ...
            <p><strong>共感:</strong> ${diary.response ? diary.response : 'なし'}</p>
            <div class="diary-time">${diary.time}</div>
        `;
...
// カレンダーの日付をクリックしたら `fetchDiaries()` を実行
document.getElementById('calendarDays').addEventListener('click', (e) => {
    if (e.target.classList.contains('calendar-day')) {
        const day = e.target.textContent.padStart(2, '0'); // 01, 02 の形式にする
        const month = (new Date().getMonth() + 1).toString().padStart(2, '0'); // 1桁月を0埋め
        const year = new Date().getFullYear();
        const dateStr = `${year}-${month}-${day}`;

        fetchDiaries(dateStr);
    }
});
